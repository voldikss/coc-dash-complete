...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  const config = workspace.getConfiguration('dash-complete')
...
export class DashCompleteProvider implements CompletionItemProvider {
  constructor() {}
  ...
  provideCompletionItems(document: TextDocument, position: Position): ProviderResult<CompletionItem[]> {
    ...
    const wordRange = doc.getWordRangeAtPosition(Position.create(position.line, position.character - 2))
    if (!wordRange) return []
    ...
    return this.gatherWords()
      .filter(word =>
        word.indexOf(preWord) < 0 &&
        preWord.indexOf(word) < 0)
